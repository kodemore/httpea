...
from .http_error import HttpNotFoundError
...
class Route:
    ...
    def match(self, uri: str) -> Optional[Route]:
        ...
        for value in matches:
            route._parameters[self._parameters_names[match_index]] = parse_qs_value(
                value
            )
            match_index += 1
...
class Router:
    ...
    def append(
        ...
        if not isinstance(route, Route):
            raise TypeError(
                f"Route must be either string or Route instance, not {type(route)}"
            )
